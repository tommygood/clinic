    ...
    sql : try {
        ...
        for (let i = 0;i < all_keys.length;i++) { // 找是否有藥品需匯入資料庫
            if (all_keys[i].includes('medicines') && !Object.values(req.body[all_keys[i]])[0] == "") {
                ...
                if (all_this_code.length == 0) { // 藥品資料庫中找不到此款藥品
                    // 就是處方籤
                    await conn.query('insert into medicines_records(`rId`, `subscript`) values(?, ?)', [req.body.rId, Object.values(req.body[all_keys[i]])[0]]);
                    continue;
                }
                // 可能有多筆相同英文名字的藥品，最後一個才是正確的
                var true_code = all_this_code[all_this_code.length-1]; 
        ...
        await conn.query('insert into diagnose_records(`rId`, `diagnose_code`, `main_sue`) values(?, ?, ?)', [req.body.rId, req.body.diagnose_code_pass, req.body.main_sue])
...
router.post('/backup', async function(req, res) {
    ...
    try {
        ...
        await conn.query('delete from backup_medicines_records where `rId` = ?;', req.body.rId);
        for (let i = 0;i < all_keys.length;i++) { // 找是否有藥品需匯入資料庫
            if (all_keys[i].includes('medicines') && !Object.values(req.body[all_keys[i]])[0] == "") {
                const all_this_code = await conn.query('select `index`, `code` from medicines where `medi_eng` = ?', Object.values(req.body[all_keys[i]])[0]); // 全部是這個 code 的藥品
                ...
                await conn.query('insert into backup_medicines_records(`rId`, `medicines_id`, `day_num`, `days`, `rule`, `mark`) values(?, ?, ?, ?, ?, ?)', [req.body.rId, true_code.index, day_num, days, Object.values(req.body[all_keys[i]])[3], Object.values(req.body[all_keys[i]])[4]]);
                const in_normal = await conn.query('select count(*) from medicines_normal where `code` = ?', true_code.code);
                var code = true_code.code;
                if (in_normal[0]['count(*)']==0) { // 還沒被放入常用的藥品
                    const all_this_code = await conn.query('select * from medicines where `code` = ?', code); // 全部是這個 code 的藥品
        ...
        await conn.query('delete from backup_diagnose_records where `rId` = ?;', req.body.rId);
        await conn.query('insert into backup_diagnose_records(`rId`, `diagnose_code`, `main_sue`) values(?, ?, ?)', [req.body.rId, req.body.diagnose_code_pass, req.body.main_sue])
        //await conn.query('update records set `end` = ? where `no` = ?', [new Date(), req.body.rId]); 
    }
    catch(error) {
        console.log(error);
