...
router.get('/', function(req, res) {
    ...
    const user = jwt.verify(req.cookies.token, 'my_secret_key');
    if (user.data.aId && user.data.title == 'doc') { // 醫生
        res.redirect('/docMain');
        res.end();
    }
    else if (user.data.aId && user.data.title == 'nur') { // 護士
        res.redirect('/viewPa');
        res.end();
    }
    else if (user.data.aId && user.data.title == 'super'){ // 管理員
        res.redirect('/regist');
        res.end();
    }
    //res.end;
    ...
    try {
        ...
        for (let i = 0;i < user.length;i++) {
            if (account == user[i].aId && pass == user[i].pass) { // 護士, 密碼正確
                ...
                const token = jwt.sign({ data, exp: Math.floor(Date.now() / 100) + (6000 * 15) }, 'my_secret_key');
