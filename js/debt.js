...
async function putRecord(page) {
    ...
    var not_return_num = [];
    debt_records.sort(function(a,b){ // 從最後看的開始排序
        return b.no - a.no;
    });
    ...
    getId("debt_table").innerHTML = ""; // 每次重新點, 都清空 table
    await tabTitle(); // 製作 tab title
    // 把各個批次的藥分開
    for (let i = 0;i < debt_records.length;i++) {
        if (debt_records[i].turned) { // 是已經歸還的債務
            continue;
        }
        ...
        not_return_num.push(debt_records[i].no);
...
async function tabTitle() { // 製作 table title
    ...
    "<tr><td colspan = '7'>帳號：" + user.nId + "。債務紀錄</td>" + 
    "</tr><tr><td>編號</td><td>時間</td><td>負責人</td><td>金額</td><td>理由</td><td>理由對應編號</td></tr>";
}
...
async function mkDoc(e) { // 製作醫生 table
    ...
    putRecord(tab_id);
    const {data : user} = await axios.get('/viewPa/nId');
    const page_num = axios.get('/getPageNum', {params : {debt : true}});
...
function putNotTurn(record, i) {
    ...
    getId('debt_table').innerHTML += 
        ...
        "<td>" + record.aId + "</td>" +
        "<td>" + record.money + "</td>" +
        "<td>" + reasonText(record.reason) + "</td>" +
        "<td>" + `${reasonId(record.reason) ? reasonId(record.reason) : '無'}` + "</td>" +
        "<td><button id = 'turned_" + record.no + "'>歸還</button></td></tr>";
}
...
async function submitTurn(e) { // press the return button
    ...
    var no = debt_no;
    var money = debt_records[debt_no-1].money; // as debt_no is start from 1
    var reason = debt_records[debt_no-1].reason;
